...
class SynonymQtApp(QMainWindow):
        ...
        if datasource is None:
            ...
            if file_name:
                for entry in self.dictionary_keys:
                    self._remove_dictionary(entry)
                self.datasource = SynonymDataSource(file_name)
            else:
                exit()

        self._init_search_configuration(column=column)
        self._init_dictionary_layout()
        self._init_export_layout()
    ...
    def _init_search_configuration(self, column: str = None):

        data_source_name = QLineEdit()
        data_source_name.setText(self.datasource.file_name)
        data_source_name.setEnabled(False)
        data_source_button = QPushButton("...")
        data_source_button.clicked.connect(lambda state: self._open_data_source_dialog())
        ...
        search_button.clicked.connect(
            ...
            self._search_synonyms(column, word_field, tolerance))
        search_button.setEnabled(False)
        word_field.textChanged[str].connect(lambda: search_button.setEnabled(word_field.text() != ""))

        self.grid.addWidget(search_button, 4, 0, 1, 2)

    def _init_dictionary_layout(self):
    ...
    def _search_synonyms(self, column: QComboBox, word_field: QLineEdit, tolerance: QSlider):
        self._add_dictionary(column.currentText(), word_field.text(), tolerance.value())

    def _add_dictionary(self, column: str, word: str, tolerance: float):
        synonyms = self.datasource.search_synonyms(column, word, tolerance)
        key = self._key(column, word, tolerance)
        if key in self.checkboxes:
            self._update_dictionary(key, synonyms)
        else:
            entry = self.Entry(column, word, tolerance, key)
            self.dictionary_keys.append(entry)
            self.checkboxes[key] = [self._check_box(s) for s in synonyms]
            w = QWidget()
            h = QHBoxLayout()
            w.setFixedHeight(50)
            w.setObjectName("" + key)
            ...
            remove_button.clicked.connect(lambda state: self._remove_dictionary(entry))
            ...
            w.mouseReleaseEvent = lambda event, key=key: self._select_dictionary(key)
            for cb in self.checkboxes[key]:
                self.synonym_layout.addWidget(cb)

            self.export_button.setEnabled(True)

        self._select_dictionary(key)

    def _update_dictionary(self, key, synonyms):
        ...
        for cb in to_remove:
            cb.deleteLater()
        new_check_boxes = [self._check_box(s) for s in to_add]
        self.checkboxes[key].extend(new_check_boxes)
        for cb in new_check_boxes:
            self.synonym_layout.addWidget(cb)

    def _select_dictionary(self, key: str):
    ...
    def _remove_dictionary(self, entry: Entry):
        if entry.key == self.selected_dictionary:
            ...
            if dict_index > 0 and dict_index == (len(self.dictionary_keys) - 1):
                self._select_dictionary(self.dictionary_keys[dict_index - 1].key)
    ...
    def _check_box(self, s: str) -> QCheckBox:
        cb = QCheckBox(s)
        cb.setParent(self.centralWidget())
        cb.setFixedHeight(30)
        return cb

    def _key(self, column: str, word: str, tolerance: float) -> str:
